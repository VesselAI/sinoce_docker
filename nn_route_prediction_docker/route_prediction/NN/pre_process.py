            ...
            if features is not None:
                for feature in features:
                    if feature == 'cog':
                        ...
                        feat = np.atleast_2d(np.array(gdf.iloc[i][feature])).T
                        data = np.concatenate((data, feat), axis=1)
...
def get_sliding_window(data: list, window_size:int =10, output_len:int=1, idx:list = None, to_torch=False,
                       cat_data=None, cont_data=None):
    """
    ...
    for i in data_idx:
        x = data[i]
        max_len = x.shape[0] - window_size - output_len + 1
        unpacked_src = [x[j:j + window_size, :] for j in range(0, max_len)]
        unpacked_trg = [x[j+window_size:j+window_size+output_len, :] for j in range(0, max_len)]
        split_src_data.extend(unpacked_src)
        split_trg_data.extend(unpacked_trg)
        if cat_data is not None:
            x_cat = [cat_data[i, :] for _ in range(len(unpacked_src))]  # fix that it's -1 as src, trg are differnt
            split_cat_data.extend(x_cat)

        if cont_data is not None:
            x_cont = [cont_data[i] for _ in range(len(unpacked_src))]  # fix that it's -1 as src, trg are differnt
            split_cont_data.extend(x_cont)
    ...
    if to_torch:
        #batch first (dim=0)
            if cat_data is not None and cont_data is None:
                return torch.cat([torch.from_numpy(item).unsqueeze(0) for item in split_src_data], dim=0).float(), \
                       torch.cat([torch.from_numpy(item).unsqueeze(0) for item in split_trg_data], dim=0).float(), \
                       torch.cat([torch.from_numpy(item).unsqueeze(0) for item in split_cat_data], dim=0).int(), \
                       None
            elif cat_data is None and cont_data is not None:
                return torch.cat([torch.from_numpy(item).unsqueeze(0) for item in split_src_data], dim=0).float(), \
                       torch.cat([torch.from_numpy(item).unsqueeze(0) for item in split_trg_data], dim=0).float(), \
                       None, \
                       torch.cat([torch.from_numpy(item).unsqueeze(0) for item in split_cont_data], dim=0).float()
            elif cat_data is not None and cont_data is not None:
                return torch.cat([torch.from_numpy(item).unsqueeze(0) for item in split_src_data], dim=0).float(), \
                       torch.cat([torch.from_numpy(item).unsqueeze(0) for item in split_trg_data], dim=0).float(), \
                       torch.cat([torch.from_numpy(item).unsqueeze(0) for item in split_cat_data], dim=0).int(), \
                       torch.cat([torch.from_numpy(item).unsqueeze(0) for item in split_cont_data], dim=0).float()
        ...
        return split_src_data, split_trg_data, split_cat_data, split_cont_data
